...
def calc_stats(data, curryr, currqtr, first, sector_val):
    ...
    def calc_chg(data, variable, variable_name, curryr, currqtr, sector_val):
        if currqtr == 4:
            period = 1
        else:
            period = currqtr + 1
    ...
    def min_max(data, variable, curryr):
    ...
    def fill_forward(data, variable, get_row):
    ...
    def calc_missing_cons_abs(data, curryr, currqtr):
    ...
    def sub_variance(dataframe, variable, variable_name, col_name, curryr, currqtr):
    ...
    def gen_variability_metrics(data_in, var_name, curryr, currqtr, first):
        data = data_in.copy()
        ...
        for x in new_cols:
            if x not in col_test:
                added_cols.append(x)
        for x in range(0, len(new_cols)):
            if new_cols[x] == "Recalc Insert":
                index_insert = x
        count_loop = 0
        for y in range(0, len(added_cols)):
            col_test.insert(index_insert + count_loop, added_cols[y])
            count_loop += 1
        data = data[col_test]
